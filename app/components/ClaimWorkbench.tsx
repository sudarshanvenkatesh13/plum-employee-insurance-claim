...
import { AlertTriangle, CheckCircle2, FileSearch, Gauge, ShieldCheck, Sparkles } from "lucide-react";
...
export function ClaimWorkbench({ policyName, testCases }: Props) {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-8 px-6 py-8">
      <header className="overflow-hidden rounded-[2rem] border border-white/80 bg-white/80 p-8 shadow-glow backdrop-blur">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-plum-100 bg-plum-50 px-4 py-2 text-sm font-semibold text-plum-600">
          ...
          <div className="grid min-w-72 grid-cols-3 gap-3 rounded-3xl bg-slate-950 p-4 text-white">
      ...
      <section className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-5 flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-slate-950">Claim Submission</h2>
              <p className="text-sm text-slate-500">Load a fixture or edit the JSON payload directly.</p>
            </div>
            <FileSearch className="h-8 w-8 text-plum-500" />
          </div>

          <label className="text-sm font-semibold text-slate-700" htmlFor="case">
            Demo scenario
          </label>
          <select
            id="case"
            className="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium outline-none ring-plum-500 transition focus:ring-2"
          ...
          <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-600">{selectedCase.description}</div>

          <textarea
            className="mt-4 h-[30rem] w-full rounded-2xl border border-slate-200 bg-slate-950 p-4 font-mono text-sm text-slate-100 outline-none ring-plum-500 transition focus:ring-2"
            value={payload}
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row">
            ...
            <button
              className="rounded-2xl border border-slate-200 bg-white px-5 py-3 font-bold text-slate-800 transition hover:bg-slate-50"
              onClick={runAllEvals}
              disabled={isProcessing}
            >
              Run 12-Case Eval
...
function DecisionPanel({ result }: { result: ProcessClaimResponse | null }) {
  if (!result) {
    return (
      <div className="flex min-h-[42rem] items-center justify-center rounded-[2rem] border border-dashed border-slate-300 bg-white/70 p-8 text-center">
        <div>
          <ShieldCheck className="mx-auto h-12 w-12 text-plum-500" />
          <h2 className="mt-4 text-2xl font-bold text-slate-950">Decision cockpit ready</h2>
          <p className="mt-2 max-w-md text-slate-500">Process a claim to see the status card, approved amount, confidence, extracted facts, and trace timeline.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
      <div className={`rounded-3xl border p-6 ${tone === "emerald" ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}>
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-sm font-bold uppercase tracking-widest text-slate-500">System output</p>
            <h2 className="mt-2 text-4xl font-black text-slate-950">{statusLabel}</h2>
            <p className="mt-3 max-w-2xl text-slate-700">{isEarlyStop ? result.message : result.reason}</p>
          </div>
      ...
      {result.status === "DECIDED" && result.line_items.length > 0 ? (
        <section className="mt-6">
          <h3 className="mb-3 text-lg font-bold text-slate-950">Line-item adjudication</h3>
          <div className="overflow-hidden rounded-2xl border border-slate-200">
            {result.line_items.map((item, index) => (
              <div key={`${item.description}-${index}`} className="grid grid-cols-[1fr_auto] gap-4 border-b border-slate-100 p-4 last:border-0">
                <div>
                  <p className="font-semibold text-slate-900">{item.description}</p>
                  <p className="text-sm text-slate-500">{item.reason ?? "No item-level note"}</p>
                </div>
                <div className="text-right">
                  <p className="font-bold text-slate-950">{currency(item.amount)}</p>
                  <p className="text-xs font-bold uppercase text-slate-500">{item.decision ?? "Observed"}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="mt-6">
        <div className="mb-3 flex items-center gap-2">
          ...
          <h3 className="text-lg font-bold text-slate-950">Explainability trace</h3>
        </div>
        <div className="space-y-3">
          {result.trace.map((event) => (
            <div key={event.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="flex items-center justify-between gap-4">
                <p className="text-sm font-bold text-slate-500">{event.agent}</p>
                <span className="rounded-full bg-white px-3 py-1 text-xs font-black text-slate-600">{event.status}</span>
              </div>
              <h4 className="mt-2 font-bold text-slate-950">{event.title}</h4>
